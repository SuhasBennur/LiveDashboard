...
- Live Data → Real‑time metrics and charts from active wells.
- Historical Data → Scrollable table of past records with filtering options.
- Analytics → Aggregated insights such as highest flow rates, average pressure/temperature, and well performance comparisons.
...
backend/
  ...
  core/
    config.py           # App configuration (CORS, FastAPI factory)
    excel_utils.py      # Excel initialization and append helpers
    data_store.py       # In-memory store for demo data
  routes/
    history.py          # /api/history endpoint
    analytics.py        # /api/analytics endpoint
  sockets/
    oil_stream.py       # WebSocket handler for live data stream
...
Demo Flow
- Start FastAPI backend → generates simulated oil well data every second.
- React frontend connects via WebSocket → displays live metrics and charts.
- Historical data is stored and can be browsed in the History tab.
- Analytics tab shows aggregated insights and well performance charts.
